  ...
  const handlePublish = async () => {
    ...
    try {
      const result = await createEventAction({
        ...
        posterUrl: null,
        coordinatorName:
          role === "ADMIN" ? "Admin Team" : "Volunteer Coordinator",
        eligibility: {
    ...
    <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full pb-12 selection:bg-primary-container/20 selection:text-primary">
      ...
      <header className="border border-border bg-card p-4 md:p-6 shadow-sm">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <h2 className="font-heading text-lg font-bold text-foreground uppercase tracking-tight">
            Create New Event
          </h2>
          <div className="flex flex-wrap items-center gap-2 md:gap-4 font-mono text-[10px] md:text-[11px] uppercase tracking-wider">
            <div
              className={`flex items-center gap-1.5 md:gap-2 ${step >= 1 ? "text-primary font-bold" : "text-muted-foreground"}`}
            >
              <span
                className={`w-5 h-5 md:w-6 md:h-6 border flex items-center justify-center text-[9px] md:text-xs ${step === 1 ? "bg-primary text-primary-foreground border-primary" : "border-border"}`}
              >
                1
              </span>
            ...
            <div
              className={`flex items-center gap-1.5 md:gap-2 ${step >= 2 ? "text-primary font-bold" : "text-muted-foreground"}`}
            >
              <span
                className={`w-5 h-5 md:w-6 md:h-6 border flex items-center justify-center text-[9px] md:text-xs ${step === 2 ? "bg-primary text-primary-foreground border-primary" : "border-border"}`}
              >
                2
              </span>
              <span>Sessions</span>
            ...
            <div className="hidden sm:block w-6 md:w-8 h-[1px] bg-border"></div>
            <div
              className={`flex items-center gap-1.5 md:gap-2 ${step >= 3 ? "text-primary font-bold" : "text-muted-foreground"}`}
            >
              <span
                className={`w-5 h-5 md:w-6 md:h-6 border flex items-center justify-center text-[9px] md:text-xs ${step === 3 ? "bg-primary text-primary-foreground border-primary" : "border-border"}`}
              >
                3
              </span>
              <span>Review</span>
      ...
      {wizardError && (
        <Alert
          variant="destructive"
          className="rounded-none border-destructive bg-destructive/5 text-destructive font-mono text-xs uppercase"
        >
      ...
      {step === 1 && (
        <div className="border border-border bg-card p-4 md:p-6 space-y-6">
          ...
          <form
            onSubmit={handleBasicSubmit(nextStepBasic)}
            className="space-y-6"
          >
            {/* Title */}
            ...
            <div className="flex flex-col gap-1">
              ...
              {basicErrors.description && (
                <span className="font-mono text-[10px] text-destructive uppercase mt-1">
                  {basicErrors.description.message}
                </span>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-1">
                ...
                {basicErrors.maxParticipants && (
                  <span className="font-mono text-[10px] text-destructive uppercase mt-1">
                    {basicErrors.maxParticipants.message}
                  </span>
                )}
              </div>
              ...
              <div className="flex flex-col gap-1 md:col-span-2">
                ...
                {basicErrors.whatsappInviteLink && (
                  <span className="font-mono text-[10px] text-destructive uppercase mt-1">
                    {basicErrors.whatsappInviteLink.message}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="border border-border bg-surface-container-low p-4 space-y-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-3">
                  {(["ALL", "STUDENTS", "FACULTY", "BOTH"] as const).map(
                    (opt) => (
                      <label
                        key={opt}
      ...
      {step === 2 && (
        <div className="border border-border bg-card p-4 md:p-6 space-y-6">
          ...
          <div className="border border-border bg-surface-container-low p-4 space-y-4">
            ...
            {sessionError && (
              <span className="font-mono text-[10px] text-destructive uppercase block">
                {sessionError}
              </span>
            )}
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-end">
              ...
              <span className="font-mono text-[10px] bg-surface-container px-2 py-1 border border-border">
            ...
            {sessions.length === 0 ? (
              ...
              <div className="border border-border bg-background divide-y divide-border">
                ...
                {sessions.map((sess, idx) => (
                  <div
                    key={idx}
                    className="flex flex-col md:grid md:grid-cols-12 gap-2 px-4 py-3 font-mono text-xs items-stretch md:items-center border-b border-border last:border-b-0 md:border-b-0"
                  >
                    <div className="md:col-span-6 font-semibold text-foreground text-sm md:text-xs">
                      {sess.title}
                    </div>
                    ...
                    <div className="md:col-span-1 text-right flex justify-end pt-2 border-t border-border/45 md:border-t-0 md:pt-0">
                      <button
                        ...
                        className="p-2 border border-destructive/20 md:border-none hover:bg-destructive/5 md:hover:bg-transparent text-primary hover:text-primary-container flex items-center justify-center gap-1.5 w-full md:w-auto font-mono text-[10px] md:text-xs uppercase cursor-pointer"
                      >
                        <Trash2 size={14} className="pointer-events-none" />
                        <span className="inline md:hidden text-destructive font-bold">
                          Delete Block
                        </span>
                      </button>
                    </div>
            ...
            <div className="border border-border p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-mono text-xs">
                ...
                <div>
                  ...
                  <span className="font-semibold text-foreground">
                    {new Date(basicInfo?.eventDate || "").toLocaleDateString()}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <span className="font-semibold text-foreground">
                    {basicInfo?.maxParticipants} Attendees
                  </span>
                </div>
                <div>
                  <span className="text-muted-foreground block uppercase">
            ...
            <div className="border border-border p-4 space-y-3">
              ...
              <div className="divide-y divide-border font-mono text-xs">
                {sessions.map((sess, idx) => (
                  <div
                    key={idx}
                    className="py-2 flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-4"
                  >
                    <div>
                      <span className="font-semibold text-foreground">
                        {sess.title}
